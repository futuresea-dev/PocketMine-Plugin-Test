...
class SavePlayerDataTask extends Task{
	...
	public function __construct(string $ip, string $username, string $xuid, string $uuid, string $data, string $dbPath){
		$this->ip = $ip;
	...
	public function onRun() : void{
		...
		$stmt = $database->prepare("INSERT INTO players (uuid, username, xuid, ip, user_data) VALUES (:uuid, :username, :xuid, :ip, :user_data)");
		...
		$stmt->bindValue(':user_data', $this->data, SQLITE3_TEXT);
		$stmt->execute();
